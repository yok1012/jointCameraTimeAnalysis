...
| ② ROI設定 | ヒートマップ上でドラッグして矩形ROIを追加・編集・削除。手動数値入力や JSON インポート/エクスポートにも対応 |
| ③ 分析実行 | ROI統計を計算し、時系列グラフ・オーバーレイ画像・クロップ画像を表示。CSV/PNG/ZIP でダウンロード可能 |
...
ROIは最大10件まで登録でき、それぞれ独立した範囲を持ちます。
...
4. ROIオーバーレイ画像・ROI切り出し画像が表示される
5. CSV / PNG / ZIP でダウンロード可能
